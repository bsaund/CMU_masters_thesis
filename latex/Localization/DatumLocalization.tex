...
\author{Shiyuan Chen$^{1}$, Brad Saund$^{2}$, Reid Simmons$^{3}$% <-this % stops a space
\thanks{$^{1}$Shiyuan Chen, $^{2}$Brad Saund, and $^{3}$Reid Simmons are with the Robotics Institute at Carnegie Mellon University}%
}

% \particles[time][index][section]
%% \DeclareDocumentCommand{\particles}{ O{} O{} O{}}{^{#2}\mathcal{P}_{#1}^{#3}}
...
%% Automated manufacturing requires high-precision localization of objects, which is currently achieved largely by human aid and the pre-design of fixed set of localization actions for robot arms. 
%% In this paper, we propose a touch-based localization approach for a potentially large and complex object with multiple datums in a 3D workspace. We use probabilistic methods to reason over the distribution of the estimated object poses in the 6-DOF configuration space under uncertainty. We introduce the \textit{datum-based particle filter} to handle intrinsic tolerances between each datum of the object while updating our belief of the particular target geometric features.
%% We describe two alternative methods for the particle filter system: one using the full joint belief and the others reasonably simplifying the belief to achieve a better ability to scale.
...
%% Thus we want to localize each referenced datum individually while reducing the uncertainty of the transformation relations, and ultimately localize the target features. 

\begin{figure}
  \centering
  \begin{subfigure}[b]{.9\linewidth}
    \includegraphics[width=\linewidth, clip, trim=8in 6in 8.5in 4in]{UnlocalizedBelief}
    \caption{Initial belief}
    \label{fig:FullBelief:Unlocalized}
  \end{subfigure}
  \hfill
  \begin{subfigure}[b]{.9\linewidth}
    \includegraphics[width=\linewidth, clip, trim=7in 7in 10in 4in]{LocalizedBelief}
...
%% In 3-dimensional workspace, there are a total of 6DOFs for a rigid body.
%% Rather than solving a interpolation/fitting problem to compute the best estimate the pose, we keep updating the belief of the probabilistic distribution regarding to the object pose based on the measurements.
...
%% We implemented our methods on our robot equipped with both high-precision touch probe and 1D laser range finder\cite{Saund2017}. 

% Directly from ICRA 2017 
%% A touch probe is a spherical tip mounted to a rigid shaft connected to the end effector capable of detecting contact. 
%% Various designs may implement a touch probe differently, for instance thresholding a value from a force sensor, displacement sensor, a pressure sensitive skin, or joint torque measurements, however the end result is a sensor which returns a binary measurement of whether the probe is in contact with an object. 
%% We make the assumption that every probe measurement action will involve the probe tip moving along a linear trajectory through space until contact is made, at which point the robot will stop as to not damage the probe. 
%% 1D laser range finder measures the linear distance between the sensor and the object. It offers faster measurements without requiring contact with the object. 
...
\centering
\begin{tikzpicture}
  \node (img) {\includegraphics[width=3in, clip, trim=1.5in 3.3in 1.5in 3.3in]{translational_error}};
  \node[left=of img, node distance=0cm, rotate=90, anchor=center,yshift=-1cm] 
       {Translational Error (cm)};
\end{tikzpicture}
\begin{tikzpicture}
  \node (img) {\includegraphics[width=3in, clip, trim=1.5in 3.3in 1.5in 3.3in]{rotational_error}};
  \node[left=of img, node distance=0cm, rotate=90, anchor=center,yshift=-1cm] 
...
\end{tikzpicture}
%% \begin{tikzpicture}
%%   \node (img) {\includegraphics[width=3in, clip, trim=0in 0in 0in 0in]{fitted_rate}};
%%   \node[left=of img, node distance=0cm, rotate=90, anchor=center,yshift=-1cm] 
%%       {Fitted Rate (\%)};
%%   \node[below=of img, node distance=0cm, yshift=1.2cm] 
%%       {Measurement Number};
...
% ???TODO??? NEW GRAPH NEEDED WITH DATA FROM BOTH PARTICLE FILTERS
...
%% This material is based upon research supported by (while Dr. Simmons was
